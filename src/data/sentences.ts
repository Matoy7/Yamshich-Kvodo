...
export async function fetchSentences(view: FeedView, userId: string): Promise<Sentence[]> {
  if (view === 'completed') {
...
export async function fetchCompletedIds(userId: string): Promise<Set<string>> {
  ...
  if (error) throw error
  return new Set(((data ?? []) as { sentence_id: string }[]).map((row) => row.sentence_id))
}

export async function createSentence(text: string, userId: string): Promise<void> {
