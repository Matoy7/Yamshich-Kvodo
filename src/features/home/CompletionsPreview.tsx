...
function place(anchor: DOMRect, height: number): Placement {
  ...
  const above = roomAbove >= Math.min(height, MAX_HEIGHT) || roomAbove >= roomBelow
...
export function CompletionsPreview({
  ...
  const { completions, loading, error, retry } = useCompletions(sentence.id)
  ...
  return createPortal(
    <div
      ...
      className={cn(
        ...
        placement ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 scale-[0.98]',
...
export function CompletionsSheet({ sentence, authorName, onClose }: SheetProps) {
  const { completions, loading, error, retry } = useCompletions(sentence.id)
  ...
  return createPortal(
    <div>
      <div onClick={onClose} className="fixed inset-0 z-40 bg-content-primary/35" aria-hidden />
