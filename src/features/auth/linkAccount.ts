...
function classify(code: string | null, description: string | null): LinkOutcome {
...
export async function consumeAccountLinkOutcome(): Promise<LinkResult | null> {
  ...
  if (failure) {
    clearOAuthError()
    return { outcome: classify(failure.code, failure.description), detail: failure.description }
  }
