...
-- Supabase normally grants these to `authenticated` automatically for new
-- tables in `public`, via default privileges. Stated explicitly so the feature
-- cannot silently fail on a project where those defaults do not apply — a
-- missing grant looks exactly like a missing table from the client.
grant select, insert, delete on public.completion_likes to authenticated;
