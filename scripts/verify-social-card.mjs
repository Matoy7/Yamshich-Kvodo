...
if (/content=["'][^"']*noindex/i.test(html)) {
  problems.push('<meta name="robots" content="noindex"> blocks preview crawlers')
}
if (existsSync('dist/robots.txt') && /Disallow:\s*\/\s*$/m.test(readFileSync('dist/robots.txt', 'utf8'))) {
  problems.push('robots.txt disallows everything, which blocks preview crawlers')
}
...
if (image) {
  ...
  if (!existsSync(local)) {
    problems.push(`og:image points at ${image} but ${local} is not in the build output`)
  } else {
    const kb = Math.round(statSync(local).size / 1024)
    if (kb > MAX_KB) problems.push(`${local} is ${kb} KB; keep it under ${MAX_KB} KB for WhatsApp`)
  }
}
