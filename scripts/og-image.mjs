...
const font = (relative) => {
  for (const base of [
    ...
    "/tmp/ogfonts/node_modules",
  ]) {
    const file = path.join(base, relative)
    if (fs.existsSync(file)) return fs.readFileSync(file).toString("base64")
  }
  throw new Error(
    `missing font ${relative} — run: npm i --no-save @fontsource/rubik @fontsource/alef`,
  )
}
...
console.log(
  `[og-image] ${path.relative(ROOT, target)} — ${WIDTH}×${HEIGHT}, ${Math.round(bytes / 1024)} KB`,
)
if (bytes > MAX_BYTES) {
  console.error(
    `[og-image] too large: ${Math.round(bytes / 1024)} KB > ${MAX_BYTES / 1024} KB. WhatsApp may drop it.`,
  )
  process.exit(1)
}
